...
#So i will be substituting the GDP estimates of the countries missing substantial data, with the new data from the Maddison project, i am aware that mixing GDP estimates can be a methodological problem, but as long as i am concerned here with GDP in relative, and not absolute terms, i believe that this will not be as much of an issue, given that i replace all of the data for the countries missing data before, with data im now getting from the Maddison project.
MaddisonGDP <- read_csv("/Users/henry/Desktop/SEMESTER 2/Digital Arkiv og Metode/Final Project/Ruling_Parties_1990s_mapping/Data/gdp-maddison-project-database.csv")
